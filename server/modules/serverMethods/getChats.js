...
function getChats(socket) {
    ...
    let combineMessagesText = `SELECT "chat".id as "chat_id", jsonb_build_object('id', "messages".id, 'message', "messages".message, 'username', "user".username, 'date', "messages".date, 'img', "user".image) as message_details, "messages".id as "message_id", (CASE when "messages".user_id=1 THEN true ELSE false END) as "owner" FROM "chat"
    ...
    let combineMessagesByChat = `SELECT "chat_id", "chat".start_date as "chat_date", jsonb_agg("foo".message_details ORDER BY foo."message_id") as chat_messages, "chat".user1, "chat".user2, "chat".active 
    from (${combineMessagesText}) as foo
    ...
    let fillUsernames = `SELECT "chat_id", "chat_date", "chat_messages", array_agg("user".username) as participants, "goo".active,
    jsonb_build_object('image', "user".image, 'name', "user".username) as "partner"
    FROM (${combineMessagesByChat}) as goo
    JOIN "user" ON "user".id = "user1" OR "user".id = "user2"
    WHERE "user".id != $1
    GROUP BY "chat_id", "chat_messages", "goo".active, "goo".chat_date, "user".id
